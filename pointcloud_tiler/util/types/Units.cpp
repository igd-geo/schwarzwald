...
constexpr static std::array<UnitPrefix, 9> BinaryPrefixes = {
    UnitPrefix{"", 1},
...
template <size_t N>
static std::string format_quantity(double quantity, uint32_t precision,
                                   const std::array<UnitPrefix, N> &prefixes) {
  if (quantity == 0.0) {
    return "0";
  }
  ...
  // want
  const auto prefix_iter =
      std::upper_bound(std::begin(prefixes), std::end(prefixes), quantity,
                       [](double value, const UnitPrefix &prefix) {
...
std::string unit::format_with_metric_prefix(double quantity,
                                            uint32_t precision) {
  return format_quantity(quantity, precision, MetricPrefixes);
}

std::string unit::format_with_binary_prefix(double quantity,
                                            uint32_t precision) {
