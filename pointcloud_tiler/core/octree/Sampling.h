...
struct RandomSortedGridSampling {
  ...
  Iter sample_points(Iter begin, Iter end, MortonIndex<MaxLevels> node_key,
                     int32_t node_level, const AABB &root_bounds,
    ...
    const auto candidate_level_in_octree =
        std::max(-1,
                 (int)std::floor(std::log2f(root_bounds.extent().x /
    ...
    const auto stable_partition_at_level = [&](int level) {
      ...
      partition_point = stable_partition_with_jumps(
          begin, end,
          [this, &num_taken_indices, level](const auto curBegin,
                                            const auto end) {
            ...
            const auto next_iter = std::partition_point(
                ...
                [taken_cell_idx, level](const auto &other_point) {
                  return other_point.morton_index.truncate_to_level(level)
                             .get() <= taken_cell_idx.get();
                });
...
struct GridCenterSampling {
  ...
  Iter sample_points(Iter begin, Iter end, MortonIndex<MaxLevels> node_key,
    ...
    const auto spacing_at_this_node =
        spacing_at_root / std::pow(2, node_level + 1);
    const auto candidate_level_in_octree =
        std::max(-1, (int)std::floor(std::log2f(root_bounds.extent().x /
                                                spacing_at_this_node)) -
                         1);
    auto partition_point = begin;
    ...
    return stable_partition_with_jumps(
        begin, end,
        [this, candidate_level_in_octree, &num_selected_points,
         &root_bounds](const auto cur_begin, const auto cur_end) {
          /*
          ...
          */
          const auto current_cell_idx =
              cur_begin->morton_index.truncate_to_level(
                  candidate_level_in_octree);
          ...
          const auto current_cell_bounds =
              get_bounds_from_morton_index(cur_begin->morton_index, root_bounds,
                                           candidate_level_in_octree + 1);
          const auto current_cell_center = current_cell_bounds.getCenter();

          const auto min_point = std::min_element(
              cur_begin, points_in_same_cell_end,
  ...
  Iter sample_points(Iter begin, Iter end, MortonIndex<MaxLevels> node_key,
    ...
    return std::stable_partition(
        begin, end, [this, &num_points_taken, &sparse_grid](const auto &point) {
          ...
          const auto accepted =
              sparse_grid.add(point.point_reference.position());
          if (!accepted)
            return false;
...
using SamplingStrategy =
    std::variant<RandomSortedGridSampling, GridCenterSampling,
                 PoissonDiskSampling, AdaptivePoissonDiskSampling>;

template <typename T, typename... Args>
SamplingStrategy make_sampling_strategy(Args &&... args) {
  return T{std::forward<Args>(args)...};
}

template <typename... Args>
SamplingStrategy make_sampling_strategy_from_name(const std::string &name,
                                                  Args &&... args) {
  if (name == "RANDOM_GRID")
 ...
 */
template <typename Iter, unsigned int MaxLevels>
Iter sample_points(SamplingStrategy &sampling_strategy, Iter begin, Iter end,
                   MortonIndex<MaxLevels> node_key, int32_t node_level,
                   const AABB &root_bounds, float spacing_at_root) {
  return std::visit(
      [&](auto &strategy) {
        return strategy.sample_points(begin, end, node_key, node_level,
                                      root_bounds, spacing_at_root);
      },
      sampling_strategy);
}