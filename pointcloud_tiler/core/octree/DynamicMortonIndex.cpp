...
parse_string_octant_based(const std::string &str, bool has_root) {
  ...
  if (depth < 0) {
    const auto error_msg =
        boost::format("Can't parse Morton index in Potree format from string "
  ...
  for (; start_iter != str.end(); ++start_iter) {
    ...
    if (octant_index < 0 || octant_index > 7) {
      const auto error_msg =
          boost::format("Can't parse Morton index from string \"%1%\" because "
...
parse_string_grid_based(const std::string &str) {
  ...
  if (tokens.size() != 4) {
    const auto error_msg =
        boost::format("Can't parse Morton index in Entwine format from string "
...
std::string
print_morton_index_octant_based(const DynamicMortonIndex &morton_index,
                                bool has_root) {
  std::stringstream ss;
  if (has_root) {
    ss << 'r';
...
size_t DynamicMortonIndex::depth() const { return _octants.size(); }

DynamicMortonIndex::Octant_t
    DynamicMortonIndex::operator[](size_t level) const {
...
bool DynamicMortonIndex::operator==(const DynamicMortonIndex &other) const {
...
tl::expected<DynamicMortonIndex, std::string> DynamicMortonIndex::parse_string(
    const std::string &str, MortonIndexNamingConvention naming_convention) {
  ...
  default:
    return tl::make_unexpected(
        std::string{"Unexpected naming_convention "} +
        std::to_string(static_cast<uint32_t>(naming_convention)));
  }
}

std::string to_string(const DynamicMortonIndex &morton_index,
