...
void LASPersistence::persist_points(PointBuffer const &points,
                                    const AABB &bounds,
...
void LASPersistence::retrieve_points(const std::string &node_name,
                                     PointBuffer &points) {
  const auto file_path = concat(_work_dir, "/", node_name, _file_extension);
  if (!std::experimental::filesystem::exists(file_path))
    return;
  LASFile las_file{file_path, LASFile::OpenMode::Read};
  pc::read_points(std::cbegin(las_file), las_file.size(),
                  las_file.get_metadata(), _point_attributes, points);
}