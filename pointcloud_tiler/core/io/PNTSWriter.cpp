  ...
  : _filePath(filePath)
{
  ...
  // FEATURE 3D Tiles Global attributes
}

PNTSWriter::~PNTSWriter()
{
  close();
}
...
void
PNTSWriter::write_points(gsl::span<PointBuffer::PointReference> points)
{
...
void
PNTSWriter::flush(const Vector3<double>& localCenter)
{
                                                                       ...
                                                                       // before this method
{
  ...
  // FEATURE Global attributes for the JSON header
...
void
transform_pnts_file_coordinates(const std::string& file_path,
                                Recenter recenter,
...
void
attributes::PositionAttribute::extractFromPoints(const PointBuffer& points)
{
  if (!points.count())
    return;
...
std::string
attributes::PositionAttribute::getAttributeNameForJSON() const
{
  return "POSITION";
}
...
uint32_t
attributes::PositionAttribute::getAlignmentRequirement() const
{
...
{
  return uint32_t(); // FEATURE Support quantized positions
}

void
attributes::RGBAAttribute::extractFromPoints(const PointBuffer& points)
{
  if (!points.count())
    return;
  if (!points.hasColors())
...
std::string
attributes::RGBAAttribute::getAttributeNameForJSON() const
{
  return "RGBA";
}
...
uint32_t
attributes::RGBAAttribute::getAlignmentRequirement() const
{
  return 1u;
}

void
attributes::RGBAttribute::extractFromPoints(const PointBuffer& points)
{
  if (!points.count())
    return;
  if (!points.hasColors())
...
std::string
attributes::RGBAttribute::getAttributeNameForJSON() const
{
  return "RGB";
}
...
uint32_t
attributes::RGBAttribute::getAlignmentRequirement() const
{
  return 1u;
}
...
std::string
attributes::IntensityAttribute::getAttributeNameForJSON() const
{
  return "INTENSITY";
}
...
uint32_t
attributes::IntensityAttribute::getAlignmentRequirement() const
{
  return 2u;
}
...
uint32_t
attributes::ClassificationAttribute::getAlignmentRequirement() const
{
  return 1u;
}
