...
LIBLASReader::readNextBatch(size_t maxBatchSize)
{
  ...
  readCurrentPoint = continueWith(readCurrentPoint,
                                  [this](size_t currentIdx,
                                         auto& positions,
                                         auto& colors,
                                         auto& normals,
  ...
  if (has_attribute(_requestedAttributes, PointAttribute::RGB)) {
    colors.reserve(batchSize);

    readCurrentPoint =
      continueWith(readCurrentPoint,
                   [this](size_t currentIdx,
                          auto& positions,
                          auto& colors,
                          auto& normals,
  ...
  if (has_attribute(_requestedAttributes, PointAttribute::Intensity)) {
  ...
  if (has_attribute(_requestedAttributes, PointAttribute::RGBFromIntensity)) {
    colors.reserve(batchSize);
  ...
  if (has_attribute(_requestedAttributes, PointAttribute::Classification)) {
    ...
    readCurrentPoint = continueWith(
      ...
      [this](size_t currentIdx,
             ...
             auto& normals,
             auto& intensities,
             auto& classifications) { classifications.push_back(point->classification); });
  }

  for (size_t idx = 0; idx < batchSize; ++idx) {
    readCurrentPoint(idx, positions, colors, normals, intensities, classifications);
  }

  pointsRead += batchSize;

  return PointBuffer{ batchSize,          std::move(positions),   std::move(colors),
                      std::move(normals), std::move(intensities), std::move(classifications) };
}

bool
LIBLASReader::isAtEnd() const
{
  return pointsRead == numPoints();
}

bool
LIBLASReader::hasAttributes(const PointAttributes& attributes,
  ...
  for (auto& attribute : attributes) {
    if (!hasAttribute(attribute)) {
      ...
      if (!missingAttributes)
        break;
      missingAttributes->insert(attribute);
    }
  }
...
Vector3<double>
LIBLASReader::getOffset() const
{
  return { header->x_offset, header->y_offset, header->z_offset };
}

bool
LIBLASReader::hasAttribute(const PointAttribute& attribute) const
{
  switch (attribute) {
...
bool
LIBLASReader::hasIntensity() const
{
...
LASPointReader::LASPointReader(const std::string& path, const PointAttributes& requestedAttributes)
  : path(path)
...
PointBuffer
LASPointReader::readPointBatch(size_t maxBatchSize)
{
...
void
LASPointReader::close()
{
  reader = nullptr;
}

long long
LASPointReader::numPoints()
{
...
AABB
LASPointReader::getAABB()
{
  return aabb;
}

Vector3<double>
LASPointReader::getScale()
{
