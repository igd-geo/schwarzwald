...
Vector3<double> setOriginToSmallestPoint(Iter points_begin, Iter points_end) {
  const auto dblMax = std::numeric_limits<double>::max();
  const auto smallestPoint = std::accumulate(
      points_begin, points_end, Vector3<double>{dblMax, dblMax, dblMax},
      [](const auto &accum, const auto &point) {
        return Vector3<double>::minByAxis(accum, point.position());
      });

  std::for_each(points_begin, points_end, [smallestPoint](auto &point) {
