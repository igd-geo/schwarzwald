...
enum class IgnoreErrors : uint32_t
{
  ...
  // error combinations
  AllFileErrors = MissingFiles | InaccessibleFiles | UnsupportedFileFormat,
...
try_parse(const std::string& str)
{
  ...
  if (iter == std::end(TOKENS_TO_LITERALS)) {
    return tl::make_unexpected((boost::format("Unrecognized token \"%1%\"") % str).str());
  }
...
inline IgnoreErrors
operator|(IgnoreErrors l, IgnoreErrors r)
{
  return static_cast<IgnoreErrors>(static_cast<uint32_t>(l) | static_cast<uint32_t>(r));
}
...
ErrorChain
chain_error(const std::exception& exception, const std::string& why);
ErrorChain
chain_error(std::string why);
ErrorChain
chain_error(const ErrorChain& reason, const std::string& why);
