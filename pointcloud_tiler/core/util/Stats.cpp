...
void dump_perf_stats(const PerformanceStats &stats,
  ...
  fs << "Prepare duration: " << (stats.prepare_duration.count() / 1000.f)
     << std::endl;
  fs << "Indexing duration: " << (stats.indexing_duration.count() / 1000.f)
     << std::endl;
