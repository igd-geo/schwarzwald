                      ...
                      std::map<std::string, std::string>& timings)
{
  ...
  const auto partition_point = filter_points_for_octree_node(all_points.begin(),
                                                             ...
                                                             sampling_strategy);
  const auto points_taken = std::distance(std::begin(all_points), partition_point);
  ...
  persistence.persist_points(
    destructure_iterator(std::begin(all_points), &IndexedPoint64::point_reference),
...
static void
process_node(octree::NodeData&& node_data,
             octree::NodeStructure const& node,
             octree::NodeStructure const& root_node,
             Func process_call,
             Taskflow& taskflow)
    ...
    if (child_node_description.data.size() >= MinPointsForAsyncProcessing) {
      ...
      taskflow
        .emplace([_child_node = std::move(child_node_description),
          ...
          process_node(std::move(_child_node.data),
                       _child_node.node,
...
Tiler::run_worker()
{
  ...
  while (_run_indexing_thread) {
    ...
    auto indexing_tasks = parallel::transform(
      std::begin(_indexing_point_cache),
      std::end(_indexing_point_cache),
      std::begin(root_node_points),
      [this](PointBuffer::PointReference point) {
        return index_point<MAX_OCTREE_LEVELS>(point, _aabb, OutlierPointsBehaviour::ClampToBounds);
      },
      taskflow,
      concurrency,
      "indexing");

    auto sort_task = taskflow
    ...
    auto process_task =
      taskflow
        .emplace([&, root_node](tf::Subflow& subflow) {
          process_node(std::move(root_node_points), root_node, root_node, process_call, subflow);
          root_node_points = {};
        })
