...
struct NodeTilingData {
  NodeTilingData() {}
  NodeTilingData(octree::NodeData points, octree::NodeStructure node,
                 octree::NodeStructure root_node)
      : points(std::move(points)), node(node), root_node(root_node) {}
...
struct TilingAlgorithmBase {
  TilingAlgorithmBase(SamplingStrategy &sampling_strategy,
                      ...
                      TilerMetaParameters meta_parameters, size_t concurrency);
  /**
   * Build an execution graph for tiling the given PointBuffer
   */
  virtual void build_execution_graph(PointBuffer &points, const AABB &bounds,
                                     tf::Taskflow &tf) = 0;

protected:
  std::vector<NodeTilingData> tile_node(
  ...
  std::vector<NodeTilingData> tile_internal_node(
      octree::NodeData &all_points, octree::NodeStructure const &node,
      octree::NodeStructure const &root_node, size_t previously_taken_points);
...
struct TilingAlgorithmV1 : TilingAlgorithmBase {
  ...
  void build_execution_graph(PointBuffer &points, const AABB &bounds,
...
private:
  ...
  //     : is_start_node(false)
  //   {}
  //   IndexedPointsForNode(bool is_start_node,
   ...
   * invocations
   */
  Octree<std::vector<util::Range<IndexedPointsIter>>>
  merge_selected_start_nodes(
