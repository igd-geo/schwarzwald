...
get_root_bounds_from_node(OctreeNodeIndex64 node_index, const AABB& node_bounds)
{
  ...
  for (uint32_t level = node_index.levels(); level > 0; --level) {
    ...
    const auto min_z = (octant & 1) ? (bounds.min.z - current_extent.z) : (bounds.min.z);
    const auto min_y = ((octant >> 1) & 1) ? (bounds.min.y - current_extent.y) : (bounds.min.y);
    const auto min_x = ((octant >> 2) & 1) ? (bounds.min.x - current_extent.x) : (bounds.min.x);
