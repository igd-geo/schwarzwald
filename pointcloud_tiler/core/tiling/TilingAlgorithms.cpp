                    ...
                    PointsPersistence& persistence)
{
   ...
   * The MortonIndices are recomputed based on the bounds of the whole octree.
   * Since we have FP inaccuracies, some of our points might be outside of the
   * current node (node_bounds). Naively computing the MortonIndices might be
   * wrong. So we clamp to the bounds of the node to make sure that our points
   * are always inside the node. But even in this case the MortonIndex
   * calculation might result in a wrong MortonIndex, because points on the edge
   * between two nodes can be in one node or the other, depending on the FP to
   * int conversion inside calculate_morton_index
   *
   * TODO
   * What we could do instead is use the given MortonIndex of the node as a
   * baseline, and only compute the lower levels based on that index. In this
   * scenario, we could potentially also get rid of clamping
   */

  // TODO This clamp to the NODES bounds (not the OCTREE bounds) should be part
  // of the persistence. Every persistence has to guarantee that it retrieves
  // points that are fully within their nodes bounds! This also means that the
  // outlier check in index_points() is irrelevant, as it only checks for
  // outliers of the OCTREE, not the NODE
  // for (auto& position : points.positions()) {
  //   position.x =
  //     std::min(node_bounds.max.x, std::max(node_bounds.min.x, position.x));
  //   position.y =
  //     std::min(node_bounds.max.y, std::max(node_bounds.min.y, position.y));
  //   position.z =
  //     std::min(node_bounds.max.z, std::max(node_bounds.min.z, position.z));
  // }
  ...
  //                                "Indexing points of node %1% failed:\n%2%")
  //                                %
  //                              node_name % ex.what())
  //                               .str() };
  // }
  ...
  // TODO Maybe sorting after retrieval is required to make LAS work?
...
void
TilingAlgorithmV1::build_execution_graph(PointBuffer& points,
                                         const AABB& bounds,
                                         tf::Taskflow& tf)
{
...
void
TilingAlgorithmV2::build_execution_graph(PointBuffer& points,
                                         const AABB& bounds,
                                         tf::Taskflow& tf)
{
  /**
...
void
TilingAlgorithmV3::build_execution_graph(PointBuffer& points,
                                         const AABB& bounds,
                                         tf::Taskflow& tf)
{
  /**
  ...
  auto sort_estimate_get_start_node =
    tf.emplace([this, bounds](tf::Subflow& subflow) {
        ...
        _level_of_start_nodes =
          estimate_start_node_level_in_octree(indexed_points, _concurrency);
...
{
  // TODO We can optimize here, for now we use a fixed level
  return 3;
  ...
  // const auto is_large_range = [](const util::Range<IndexedPointsIter>& range)
  // -> bool {
  ...
  //       partition_points_into_child_octants(std::begin(range),
  //       std::end(range), level);
  ...
  // constexpr uint32_t MAX_LEVEL = 10;
  // constexpr float MIN_SCORE = 0.75f;
                                           ...
                                           const AABB& root_bounds)
{
  ...
  // TOOD For 3D Tiles, reconstructed nodes should have their children be
  // 'REPLACE' instead of 'ADD'
  // TODO2 Or should they? REPLACE would mean that a single child node replaces
  // the full parent node, which is probably not what we want :/
