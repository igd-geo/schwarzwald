 ...
 * octree index
 */
template <unsigned int MaxLevels> struct IndexedPoint {
...
bool operator<(const IndexedPoint<MaxLevels> &l,
               const IndexedPoint<MaxLevels> &r) {
...
/**
 * Returns the root bounds of the octree that the given node belongs to, based
 * on the bounds of the node
 */
 ...
 */
template <unsigned int MaxLevels>
MortonIndex<MaxLevels> calculate_morton_index(const Vector3<double> &position,
                                              const AABB &node_bounds) {
  ...
  const auto key_val =
      (expanded_bits_z) | (expanded_bits_y << 1) | (expanded_bits_x << 2);
  return MortonIndex<MaxLevels>{static_cast<DataType_t>(key_val)};
}

template <unsigned int MaxLevels>
MortonIndex<MaxLevels>
calculate_morton_index_naive(const Vector3<double> &position,
                             const AABB &node_bounds) {
...
template <unsigned int MaxLevels>
AABB get_bounds_from_morton_index(const MortonIndex<MaxLevels> &key,
                                  const AABB &root_bounds,
...
void calculate_morton_indices_for_points(
    PointsIter points_begin, //*points_begin is Vector3<double>
    PointsIter points_end, KeysIter keys_begin, const AABB &octree_bounds) {
  std::transform(
      points_begin, points_end, keys_begin, [&](const auto &position) {
...
index_point(PointBuffer::PointReference point, const AABB &bounds,
            OutlierPointsBehaviour outlier_points_behaviour) {
  const auto is_outlier = [](const Vector3<double> &position,
                             const AABB &bounds) {
  ...
  return IndexedPoint<MaxLevels>{
      point, calculate_morton_index<MaxLevels>(position, bounds)};
}
...
void index_points(PointBuffer::PointIterator points_begin,
                  PointBuffer::PointIterator points_end,
                  OutIter indexed_points_begin, const AABB &bounds,
                  OutlierPointsBehaviour outlier_points_behaviour) {
  std::transform(points_begin, points_end, indexed_points_begin,
                 [&bounds, outlier_points_behaviour](
                     PointBuffer::PointReference point_reference) {
                   return index_point<MaxLevels>(point_reference, bounds,
                                                 outlier_points_behaviour);
                 });
}
...
Iter filter_points_for_octree_node(
    Iter points_begin, // *Iter is IndexedPoint<MaxLevels>
    Iter points_end, MortonIndex<MaxLevels> node_key, int32_t node_level,
    const AABB &root_bounds, float spacing_at_root,
    SamplingStrategy &sampling_strategy) {
  assert(points_end >= points_begin);
  return sample_points(sampling_strategy, points_begin, points_end, node_key,
                       node_level, root_bounds, spacing_at_root);
}
...
partition_points_into_child_octants(Iter begin, Iter end,
  ...
  for (uint8_t octant = 0; octant < 8; ++octant) {
    ...
    const auto current_end = std::find_if(
        current_begin, end,
        [octant, level_to_partition_at](const auto &indexed_point) {
          return indexed_point.morton_index.get_octant_at_level(
