...
namespace Catch {
template <unsigned int Levels> struct StringMaker<MortonIndex<Levels>> {
  static std::string convert(MortonIndex<Levels> const &value) {
...
static V3
position_from_octant_indices(const std::array<uint8_t, Levels> &indices,
                             const AABB &bounds) {
...
TEST_CASE("Position from octree indices computed correctly",
          "[position_from_octant_indices]") {
...
TEST_CASE("Octree keys for first level are computed correctly",
          "[calculate_morton_indexs]") {
  ...
  calculate_morton_indices_for_points<1>(points.positions().begin(),
                                         points.positions().end(),
                                         octree_keys.begin(), bounds);
...
TEST_CASE("Octree key for multiple levels is computed correctly",
          "[calculate_morton_indexs]") {
  ...
  std::vector<V3> positions = {
      position_from_octant_indices<Levels>(expected_octants, bounds)};

  PointBuffer points{1ull, positions};

  std::vector<MortonIndex<Levels>> octree_indices;
  octree_indices.resize(points.count());
  calculate_morton_indices_for_points<Levels>(points.positions().begin(),
                                              points.positions().end(),
                                              octree_indices.begin(), bounds);
...
TEST_CASE("Octree keys for sub-range computed correctly",
          "[calculate_morton_indexs]") {
  std::vector<V3> positions = {
      V3{0.25, 0.25, 0.25}, // octant 0
      V3{0.25, 0.25, 0.75}, // octant 1
  ...
  octree_keys = {};
  octree_keys.resize(points.count());
  calculate_morton_indices_for_points<1>(points.positions().begin() + 2,
                                         points.positions().end(),
                                         octree_keys.begin() + 2, bounds);
...
TEST_CASE("Octree point filtering works correctly",
  ...
  for (size_t idx = 0; idx < points.count(); ++idx) {
    ...
    points_and_keys.push_back(
        IndexedPoint<Levels>{point_ref, octree_indices[idx]});
  }

  // Make sure the indexed points are sorted in ascending order, otherwise the
  // filter algorithm doesn't work
  std::sort(points_and_keys.begin(), points_and_keys.end(),
            [](const auto &l, const auto &r) {
              return l.morton_index.get() < r.morton_index.get();
            });

  MortonIndex<Levels> root_key;
  const auto partition_point_at_l0 = filter_points_for_octree_node(
      points_and_keys.begin(), points_and_keys.end(), root_key, 0, bounds,
      SideLength, sampling_strategy);

  std::vector<V3> expected_positions = {
      V3{0.5, 0.5, 0.5},   V3{0.5, 0.5, 16.5},  V3{0.5, 16.5, 0.5},
      V3{0.5, 16.5, 16.5}, V3{16.5, 0.5, 0.5},  V3{16.5, 0.5, 16.5},
      V3{16.5, 16.5, 0.5}, V3{16.5, 16.5, 16.5}};

  const auto num_selected_points =
      std::distance(points_and_keys.begin(), partition_point_at_l0);
  REQUIRE(num_selected_points == 8);

  std::vector<V3> actual_positions;
  actual_positions.resize(expected_positions.size());
  std::transform(points_and_keys.begin(), partition_point_at_l0,
                 actual_positions.begin(), [](const auto &point_and_key) {
                   return point_and_key.point_reference.position();
                 });
...
// TODO For now, we disabled the max_points_per_node parameter as it gives
// visual artifacts for the grid sampling methods due to the sorting (i.e. it
// potentially skips large regions of points because it proceeds in sorted
// order) Evaluate if this always makes sense

// TEST_CASE("Octree point filtering obeys the max_points_per_node parameter",
//           "[filter_points_for_octree_node]")
// {
//   // Generate a 3D grid of points where the spacing is a power of 0.5 times
//   the octree size
...
//     points.positions().begin(), points.positions().end(),
//     octree_indices.begin(), bounds);

//   // Create a vector of PointReferences with their associated MortonIndexs
//   for filtering std::vector<IndexedPoint<Levels>> points_and_keys;
//   points_and_keys.reserve(points.count());
//   for (size_t idx = 0; idx < points.count(); ++idx) {
//     auto point_ref = *(points.begin() + idx);
//     points_and_keys.push_back(IndexedPoint<Levels>{ point_ref,
//     octree_indices[idx] });
//   }

//   // Make sure the indexed points are sorted in ascending order, otherwise
//   the filter algorithm
//   // doesn't work
//   std::sort(points_and_keys.begin(), points_and_keys.end(), [](const auto& l,
//   const auto& r) {
//     return l.morton_index.get() < r.morton_index.get();
//   });
...
//   const auto num_selected_points = std::distance(points_and_keys.begin(),
//   partition_point_at_l0); REQUIRE(num_selected_points ==
//   expected_points_count);
// }

TEST_CASE("Octree point filtering is stable",
          "[filter_points_for_octree_node]") {
  ...
  std::generate_n(std::back_inserter(rnd_points), NumPoints, [&]() {
    return V3{static_cast<double>(dist(rnd)), static_cast<double>(dist(rnd)),
  ...
  const auto taken_points_first_non_sorted_iter =
      std::adjacent_find(points_and_keys.begin(), partition_point_at_l0,
                         [](const auto &l, const auto &r) {
                           return l.morton_index.get() > r.morton_index.get();
                         });
  ...
  const auto first_non_sorted_pair_iter =
      std::adjacent_find(partition_point_at_l0, points_and_keys.end(),
                         [](const auto &l, const auto &r) {
                           return l.morton_index.get() > r.morton_index.get();
                         });
  const auto non_taken_points_are_sorted =
      first_non_sorted_pair_iter == points_and_keys.end();
  REQUIRE(non_taken_points_are_sorted);
}

TEST_CASE(
    "Partitioning points at root level into child octants works correctly",
  ...
  for (size_t idx = 0; idx < positions.size(); ++idx) {
    indexed_points.push_back(
        IndexedPoint<Levels>{*(points.begin() + idx), octree_indices[idx]});
  }
  ...
  const auto actual_ranges = partition_points_into_child_octants(
      indexed_points.begin(), indexed_points.end(), 0);
...
TEST_CASE(
    "Partitioning points at non-root level into child octants works correctly",
    "[partition_points_into_child_octants]") {
  ...
  std::vector<V3> positions;
  std::transform(octant_indices.begin(), octant_indices.end(),
                 std::back_inserter(positions), [&](const auto &indices) {
                   return position_from_octant_indices<Levels>(indices, bounds);
                 });
  ...
  std::array<std::pair<int, int>, 8> expected_ranges = {
      std::pair<int, int>{0, 2}, std::pair<int, int>{2, 2},
      std::pair<int, int>{2, 2}, std::pair<int, int>{2, 3},
      std::pair<int, int>{3, 3}, std::pair<int, int>{3, 5},
      std::pair<int, int>{5, 6}, std::pair<int, int>{6, 6}};

  const auto actual_ranges = partition_points_into_child_octants(
      indexed_points.begin(), indexed_points.end(), 4);

  for (size_t idx = 0; idx < 8; ++idx) {
    const auto &cur_range = actual_ranges[idx];
    const auto actual_start_offset =
        std::distance(indexed_points.begin(), cur_range.begin());
    const auto actual_end_offset =
        std::distance(indexed_points.begin(), cur_range.end());
    REQUIRE(actual_start_offset == expected_ranges[idx].first);
    REQUIRE(actual_end_offset == expected_ranges[idx].second);
  }
}

TEST_CASE("Getting octant bounds from AABB works correctly",
          "[get_octant_bounds]") {
...
TEST_CASE("Partitioning points into child octants ensures points are still "
  ...
  const auto partitioned_points_at_l0 = partition_points_into_child_octants(
      indexed_points.begin(), indexed_points.end(), 0);
...
TEST_CASE("get_bounds_from_morton_index with one level works",
