          ...
          "[calculate_morton_indexs]")
{
  ...
  std::vector<V3> positions = { position_from_octant_indices<Levels>(
    expected_octants, bounds) };
          ...
          "[filter_points_for_octree_node]")
{
  ...
  std::sort(points_and_keys.begin(),
            points_and_keys.end(),
  ...
  std::transform(points_and_keys.begin(),
                 partition_point_at_l0,
                 actual_positions.begin(),
                 [](const auto& point_and_key) {
...
TEST_CASE("Octree point filtering is stable", "[filter_points_for_octree_node]")
{
  ...
  const auto taken_points_first_non_sorted_iter =
    std::adjacent_find(points_and_keys.begin(),
                       partition_point_at_l0,
                       [](const auto& l, const auto& r) {
                         return l.morton_index.get() > r.morton_index.get();
                       });
  ...
  const auto first_non_sorted_pair_iter =
    std::adjacent_find(partition_point_at_l0,
                       points_and_keys.end(),
                       [](const auto& l, const auto& r) {
                         return l.morton_index.get() > r.morton_index.get();
                       });
  ...
  "[partition_points_into_child_octants]")
{
  ...
  std::vector<V3> positions;
  std::transform(octant_indices.begin(),
                 octant_indices.end(),
