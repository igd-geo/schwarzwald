...
static PointBuffer generate_random_points(size_t count, const AABB &bounds) {
  std::mt19937 mt;
  ...
  std::generate_n(std::back_inserter(positions), count,
                  [&]() -> Vector3<double> {
...
point_references_from_indexed_points(Iter begin, Iter end) {
  ...
  std::transform(
      begin, end, std::back_inserter(point_references),
      [](const auto &indexed_point) { return indexed_point.point_reference; });
  return point_references;
}

static void
compare_points(const std::vector<PointBuffer::PointReference> &source,
               const PointBuffer &target) {
  ...
  for (size_t idx = 0; idx < source.size(); ++idx) {
    ...
    max_distance =
        std::max(max_distance,
                 source[idx].position().distanceTo(target.positions()[idx]));
  }
...
TEST_CASE("LASPersistence store and retrieve to LAS does not modify points") {
  ...
  index_points<MortonIndex64Levels>(std::begin(points), std::end(points),
                                    std::back_inserter(indexed_points), bounds,
  ...
  const auto octant_partitions = partition_points_into_child_octants(
      std::begin(indexed_points), std::end(indexed_points), 0);
  ...
  for (auto &octant_range : octant_partitions) {
    ...
    const auto node_bounds =
        get_bounds_from_morton_index(points_begin->morton_index, bounds, 0);

    auto point_references =
        point_references_from_indexed_points(points_begin, points_end);
    persistence.persist_points(std::begin(point_references),
                               std::end(point_references), node_bounds,
                               node_name);
