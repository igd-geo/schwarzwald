...
/**
 * The constructor needs to scan the whole PTX file to find out the bounding
 * box. Unluckily.
 * TODO: during the scan all the points are read and transformed. Afterwards,
 * during loading the points are read again and transformed again. It should be
 * nice to save the transformed points in a temporary file. That would mean a
 * LAS file or something similar. Unuseful. It's better to convert the PTX to
 * LAS files.
 * TODO: it seems theat the PTXPointReader is asked to produce the bounding box
 * more than once. Maybe it should be saved somewhere. Chez moi, scanning 14m
 * points needs 90 secs. The process speed of the PTX file is about 1m points
 * every 50 secs.
 */
...
void PTXPointReader::scanForAABB() {
      ...
      while (true) {
        ...
        if (4 == split.size() || 7 == split.size()) {
          ...
          if (0.5 != intensity) {
            ...
            count++;
            if (0 == count % 1000000)
              cout << "AABB-SCANNING: " << count << " points; " << currentChunk
                   << " chunks" << endl;
          }
        } else {
          break;
  ...
  counts[path] = count;
  AABB lAABB(Vector3<double>(minx, miny, minz),
             Vector3<double>(maxx, maxy, maxz));
  PTXPointReader::aabbs[path] = lAABB;
}

bool PTXPointReader::loadChunk(fstream *stream, long currentChunk,
                               double tr[16]) {
  vector<double> split;
...
PointBuffer PTXPointReader::readPointBatch(size_t maxBatchSize) {
