...
PotreeArguments parseArguments(int argc, char **argv) {
  ...
  args.addArgument("page-template",
                   "directory where the web page template is located.");
  ...
  args.addArgument("input-format,f",
                   "Input format. xyz: cartesian coordinates as floats, rgb: "
                   "colors as numbers, i: intensity as number");
  ...
  args.addArgument("aabb",
                   "Bounding cube as \"minX minY minZ maxX maxY maxZ\". If not "
  ...
  args.addArgument("material",
                   "RGB, ELEVATION, INTENSITY, INTENSITY_GRADIENT, "
                   "CLASSIFICATION, RETURN_NUMBER, SOURCE, LEVEL_OF_DETAIL");
  ...
  if (args.has("aabb")) {
    ...
    for (size_t p = 0, q = 0; p != strAABB.npos; p = q)
      aabbValues.push_back(
          atof(strAABB
                   .substr(p + (p != 0),
                           (q = strAABB.find(sep, p + 1)) - p - (p != 0))
                   .c_str()));
  ...
  if (a.source.size() == 0 && args.has("list-of-files")) {
    ...
    if (fs::exists(fs::path(a.listOfFiles))) {
      ...
      cerr << "ERROR: specified list of files not found: '" << a.listOfFiles
           << "'" << endl;
      exit(1);
    }
  }
  ...
  vector<string> validMaterialNames = {"RGB",
                                       "ELEVATION",
                                       "INTENSITY",
                                       "INTENSITY_GRADIENT",
                                       "CLASSIFICATION",
                                       ...
                                       "SOURCE",
                                       "LEVEL_OF_DETAIL"};
  if (std::find(validMaterialNames.begin(), validMaterialNames.end(),
                a.material) == validMaterialNames.end()) {
    cout << args.usage();
    cout << endl;
    cout << "ERROR: "
