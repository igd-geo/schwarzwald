  ...
  bool readNextPoint() {
    ...
    while (getline(stream, line)) {
      ...
      if (tokens.size() != format.size()) {
        ...
        if (linesSkipped == 0) {
          cout << "some lines may be skipped because they do not match the "
                  "given format: '"
               << format << "'" << endl;
        }
      ...
      for (const auto &f : format) {
        ...
        if (f == 'x') {
          ...
          r = (unsigned char)(255.0f * (stof(token) - colorOffset) /
                              colorScale);
        } else if (f == 'g') {
          g = (unsigned char)(255.0f * (stof(token) - colorOffset) /
                              colorScale);
        } else if (f == 'b') {
          b = (unsigned char)(255.0f * (stof(token) - colorOffset) /
                              colorScale);
        } else if (f == 'i') {
          intensity = (unsigned short)(65535 * (stof(token) - intensityOffset) /
                                       intensityScale);
        } else if (f == 's') {
          // skip
        } else if (f == 'X') {
