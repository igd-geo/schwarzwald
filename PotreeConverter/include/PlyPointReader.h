...
class PlyPointReader : public PointReader {
 ...
 public:
  PlyPointReader(string file)
    ...
    std::regex rProperty(
        "^property "
        "(char|int8|uchar|uint8|short|int16|ushort|uint16|int|int32|uint|"
        "uint32|float|float32|double|float64) (\\w*)");
  ...
  bool readNextPoint() {
    ...
    if (format == PLY_FILE_FORMAT_ASCII) {
      ...
      for (const auto &prop : vertexElement.properties) {
        ...
        if (prop.name == "x" &&
            prop.type.name == plyPropertyTypes["float"].name) {
          x = stof(token);
        } else if (prop.name == "y" &&
                   prop.type.name == plyPropertyTypes["float"].name) {
          y = stof(token);
          ...
          z = stof(token);
                   ...
                   prop.type.name == plyPropertyTypes["double"].name) {
          x = stod(token);
        } else if (prop.name == "y" &&
                   prop.type.name == plyPropertyTypes["double"].name) {
          y = stod(token);
                   ...
                   prop.type.name == plyPropertyTypes["double"].name) {
          z = stod(token);
        } else if (std::find(plyRedNames.begin(), plyRedNames.end(),
                             prop.name) != plyRedNames.end() &&
                   prop.type.name == plyPropertyTypes["uchar"].name) {
          r = (unsigned char)stof(token);
        } else if (std::find(plyGreenNames.begin(), plyGreenNames.end(),
                             prop.name) != plyGreenNames.end() &&
                   prop.type.name == plyPropertyTypes["uchar"].name) {
          g = (unsigned char)stof(token);
        } else if (std::find(plyBlueNames.begin(), plyBlueNames.end(),
                             prop.name) != plyBlueNames.end() &&
                   prop.type.name == plyPropertyTypes["uchar"].name) {
