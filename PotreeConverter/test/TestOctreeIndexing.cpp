...
position_from_octant_indices(const std::array<uint8_t, Levels>& indices, const AABB& bounds)
{
  ...
  for (auto octant : indices) {
    if ((octant & 0b1)) {
      octant_bounds.min.z += octant_bounds.size.z / 2;
    } else {
      octant_bounds.max.z -= octant_bounds.size.z / 2;
    }
    if ((octant & 0b10)) {
      octant_bounds.min.y += octant_bounds.size.y / 2;
    } else {
      octant_bounds.max.y -= octant_bounds.size.y / 2;
    }
    if ((octant & 0b100)) {
      octant_bounds.min.x += octant_bounds.size.x / 2;
    } else {
      octant_bounds.max.x -= octant_bounds.size.x / 2;
    }
    octant_bounds.size = octant_bounds.max - octant_bounds.min;
  }
...
TEST_CASE("Octree keys for first level are computed correctly", "[calculate_octree_keys]")
{
  ...
  std::vector<OctreeNodeKey<1>> octree_keys;
  octree_keys.resize(points.count());
  calculate_octree_keys_for_points<1>(
...
TEST_CASE("Octree key for multiple levels is computed correctly", "[calculate_octree_keys]")
{
  ...
  std::vector<OctreeNodeKey<Levels>> octree_indices;
  octree_indices.resize(points.count());
  calculate_octree_keys_for_points<Levels>(
...
TEST_CASE("Octree keys for sub-range computed correctly", "[calculate_octree_keys]")
{
  std::vector<V3> positions = {
    V3{ 0.25, 0.25, 0.25 }, // octant 0
    V3{ 0.25, 0.25, 0.75 }, // octant 1
...
TEST_CASE("Octree point filtering works correctly", "[filter_points_for_octree_node]")
{
  ...
  // Create a vector of PointReferences with their associated OctreeNodeKeys for filtering
  ...
  OctreeNodeKey<Levels> root_key;
  const auto partition_point_at_l0 = filter_points_for_octree_node(
    points_and_keys.begin(), points_and_keys.end(), bounds, root_key, 0, SideLength, 16);
...
TEST_CASE("Octree point filtering is stable", "[filter_points_for_octree_node]")
{
  ...
  OctreeNodeKey<Levels> root_key;
  const auto partition_point_at_l0 = filter_points_for_octree_node(
    points_and_keys.begin(), points_and_keys.end(), bounds, root_key, 0, 1024, 16);
          ...
          "[partition_points_into_child_octants]")
{
  ...
  Potree::AABB bounds{ V3{ 0, 0, 0 }, V3{ 4, 4, 4 } };
          ...
          "[partition_points_into_child_octants]")
{
  ...
  Potree::AABB bounds{ V3{ 0, 0, 0 }, V3{ 32, 32, 32 } };
...
TEST_CASE("get_bounds_from_octree_key with one level works", "[get_bounds_from_octree_key]")
{
  OctreeNodeKey64 key;
  ...
  const auto actual_bounds = get_bounds_from_octree_key(key, bounds, 1);
  REQUIRE(expected_bounds == actual_bounds);
}

TEST_CASE("get_bounds_from_octree_key with multiple levels works", "[get_bounds_from_octree_key]")
{
  OctreeNodeKey64 key;
  ...
  const auto actual_bounds = get_bounds_from_octree_key(key, bounds, 3);
  REQUIRE(expected_bounds == actual_bounds);
}