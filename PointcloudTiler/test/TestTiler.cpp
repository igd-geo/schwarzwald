...
TEST_CASE("Tiler works", "[Tiler]")
{
  ...
  for (auto& kv : processed_points) {
    ...
    const auto node_bounds = get_bounds_from_morton_index(node_key, bounds, node_level);
...
TEST_CASE("Tiler with PossionDisk sampling", "[Tiler]")
{
  ...
  const auto is_leaf_node = [](const std::string& node_name, const auto& all_nodes) {
  ...
  for (auto& kv : processed_points) {
    ...
    const auto node_key = from_string<21>(node_name);
    const auto node_level = static_cast<int32_t>(node_name.size()) - 2; // Node level, root = -1
    const auto node_bounds = get_bounds_from_morton_index(node_key, bounds, node_level + 1);
    ...
    if (!is_leaf) {
      REQUIRE(points_obey_minimum_distance(points.positions(), node_bounds, min_distance_at_node));
    }
    ...
    if (!points_all_contained) {
      UNSCOPED_INFO("Position " << fail_point.x << " " << fail_point.y << " " << fail_point.z
