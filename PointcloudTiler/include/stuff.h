...
AABB
readAABB(std::string fIn, int numPoints);

AABB
readAABB(std::string fIn);
...
std::vector<fs::path>
get_all_files_in_directory(const std::string& dir_path, Recursive recursive = Recursive::No);
...
keys(std::map<Key, Value, Compare, Alloc> const& map)
{
  ...
  std::transform(std::begin(map), std::end(map), std::back_inserter(_keys), [](const auto& kv) {
    return kv.first;
  });
  return _keys;
}
...
template<typename Key, typename Value, typename Compare, typename Alloc>
std::vector<Value>
values(std::map<Key, Value, Compare, Alloc> const& map)
{
  std::vector<Value> _values;
  _values.reserve(map.size());
  std::transform(std::begin(map), std::end(map), std::back_inserter(_values), [](const auto& kv) {
    return kv.second;
  });
  return _values;
}
