...
/**
 * Sampling strategy that partitions a node into an even grid and takes the first point to fall into
 * each grid cell
 */
struct RandomSortedGridSampling
{
                     ...
                     float spacing_at_root)
  {
    ...
    // candidate_level_in_octree is the last level in the octree at which the node sidelength is >=
    // spacing we use floor() here because this guarantees that we always get a node with sidelength
    // >= spacing. Sadly this means that there might be edge cases where the spacing is just a bit
    // too large to fit into a node (e.g. sidelength = 2, spacing = 2.1) If we don't want that, we
    // could use round() which resolves these cases while at the same time dropping the guarantee
    // for >= spacing. Also, round(log(...)) might not split fair. If there are sidelengths 4 and 2,
    // we would expect that spacing >= 3 rounds to node with sidelength 4, while < 3 rounds to 2.
    // However, log_2(4/2.9)~=0.464, which will still round to the larger node
    ...
    const auto stable_partition_at_level = [&](int level) {
      // TODO stable_partition looks at all elements, but since the elements are sorted, once we
      // found the first point
      // in a grid cell, we can skip to the next grid cell. We know the index of the next grid cell
      // (just increment the level of the current cell by one)
      // std::unordered_set<uint64_t> taken_indices;
      // partition_point =
      //   std::stable_partition(begin, end, [this, &taken_indices, level](const auto&
      //   indexed_point) {
      //     if (taken_indices.size() >= _max_points_per_node)
      //       return false;

      //     const auto significant_bits = indexed_point.morton_index.truncate_to_level(level);
      ...
      partition_point = stable_partition_with_jumps(
        begin, end, [this, &num_taken_indices, level](const auto curBegin, const auto end) {
          // Take the first point, then look for the next point that falls into a different
          // cell up to the current level
          ...
          //   std::find_if(taken_iter + 1, end, [taken_cell_idx, level](const auto& other_point) {
...
/**
 * Sampling strategy that partitions a node into an even grid and takes the point closest to the
 * center of each cell
 */
                     ...
                     float spacing_at_root)
  {
    ...
    return stable_partition_with_jumps(
      ...
      [this, candidate_level_in_octree, &num_selected_points, &root_bounds](const auto cur_begin,
                                                                            const auto cur_end) {
        /*
        HACK truncate_to_level shifts down but it should just mask away the lower levels. This
        causes bugs because the new key starts at 'candidate_level_in_octree', but
        get_bounds_from_morton_index starts at level zero up to 'candidate_level_in_octree', which
        of course is all zeroes...
        */
        ...
        //     return other_point.morton_index.truncate_to_level(candidate_level_in_octree)
...
struct PoissonDiskSampling
{
                     ...
                     float spacing_at_root)
  {
    ...
    //   begin,
    //   end,
    //   [this, candidate_level_in_octree, &num_points_taken, &root_bounds, &sparse_grid](
    ...
    //     const auto points_in_current_cell = std::distance(cur_begin, points_in_same_cell_end);
    //     std::uniform_int_distribution<int64_t> dist{ 0, points_in_current_cell };
    //     const auto point_to_take_idx = dist(_rnd);
    //     const auto point_to_take = cur_begin + point_to_take_idx;

    //     if (!sparse_grid.add(point_to_take->point_reference.position())) {
    //       // TODO We could try a couple of points instead
    //       return std::make_pair(points_in_same_cell_end, points_in_same_cell_end);
    //     }

    //     ++num_points_taken;
    //     if (num_points_taken == _max_points_per_node) {
    //       return std::make_pair(point_to_take, cur_end);
    //     }

    //     return std::make_pair(point_to_take, points_in_same_cell_end);
    //   });
...
/**
 * Sampling strategy that takes points with a minimum distance (implemented using a fast approximate
 * algorithm)
 */
struct FastPoissonDiskSampling
{
  explicit FastPoissonDiskSampling(size_t max_points_per_node);
                     ...
                     float spacing_at_root)
  {
    ...
    const auto spacing_at_this_node = spacing_at_root / std::pow(2, node_level + 1);
    // const auto level_with_sidelength_half_spacing = std::max(
    // -1, (int)std::floor(std::log2f(root_bounds.size.x / (2 * spacing_at_this_node))) - 1);
    ...
    return stable_partition_with_jumps(begin, end, [&](const auto cur_begin, const auto cur_end) {
      // const auto current_cell_idx =
      // cur_begin->morton_index.truncate_to_level(level_with_sidelength_half_spacing);

      // Take the current point, search for next point that is outside of min distance
...
/**
 * Sampling that takes the next point (in Z-order sorting) that is further away than
 * the minimum distance from the previously selected point
 */
...
make_sampling_strategy_from_name(const std::string& name, Args&&... args)
{
  ...
  if (name == "MIN_DISTANCE_FAST")
    return FastPoissonDiskSampling{ std::forward<Args>(args)... };

  throw std::runtime_error{ "Unrecognized sampling strategy name \"" + name + "\"" };
}

/**
 * Sample points for the given node from a range of points using the given sampling strategy.
 * Returns a partition point in the range of points where [begin, partition_point) contains all
 * sampled points and [partition_point, end) all remaining points. Partitioning is stable
 */
template<typename Iter, unsigned int MaxLevels>
Iter
