...
public:
  ...
  /// </summary>
  template <typename U> static Vector3<U> cast(const Vector3<T> &source) {
  ...
  friend boost::iostreams::filtering_ostream &
  operator<<(boost::iostreams::filtering_ostream &output,
             const Vector3<T> &value) {
    output << value.x << value.y << value.z;
    return output;
  }

  friend boost::iostreams::filtering_istream &
  operator>>(boost::iostreams::filtering_istream &is, Vector3<T> &value) {
    is >> value.x;
    is >> value.y;
    is >> value.z;
    return is;
