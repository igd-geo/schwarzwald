  ...
  for (; start_iter != str.end(); ++start_iter) {
    ...
    if (octant_index < 0 || octant_index > 7) {
      const auto error_msg =
        boost::format(
          "Can't parse Morton index from string \"%1%\" because of unexpected character '%2%'!") %
        str % octant_char;
      return tl::make_unexpected(error_msg.str());
    }
...
parse_string_grid_based(const std::string& str)
{
  ...
  if (tokens.size() != 4) {
    const auto error_msg =
      boost::format(
        "Can't parse Morton index in Entwine format from string \"%1%\" because it is malformed!") %
      str;
    return tl::make_unexpected(error_msg.str());
  }
  ...
  try {
    ...
    const auto error_msg =
      boost::format("Can't parse Morton index in Entwine format from string \"%1%\": %2%") % str %
      parse_error.what();
    return tl::make_unexpected(error_msg.str());
  }
