...
LASInputIterator::LASInputIterator(LASFile const& las_file, size_t index)
  ...
  , _size(las_file.size())
{
// Index must be a valid index inside the file. For end iterator, use default constructor!
#ifdef DEBUG
...
void LASOutputIterator::operator*() {}
...
LASFile::flush()
{
  ...
  // I don't see another way to flush using LASzip but to close and re-open the writer...
  close();
...
las_file_has_attribute(laszip_header const& header, PointAttribute const& attribute)
{
  switch (attribute.ordinal) {
    ...
    case attributes::NORMAL_SPHEREMAPPED:
      return false; // TODO We could look in the additional data fields, but only from the header we
                    // can't tell, so this requires an API change
                ...
                PointBuffer& points)
{
  ...
  //                         has_attribute(attributes, attributes::NORMAL_SPHEREMAPPED);
  ...
  for (size_t idx = 0; idx < to_read_count; ++idx, ++begin) {
    auto& point = *begin;
    positions.push_back(las::position_from_las_point(point, header));
    if (has_colors) {
