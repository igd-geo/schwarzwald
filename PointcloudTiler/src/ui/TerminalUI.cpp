...
static void
terminal_blit(const TerminalRenderBuffer& source,
...
UIState::get_messages() const
{
  return _messages;
}

void
UIState::push_message(std::string message)
{
...
TerminalRenderBuffer::at(uint32_t x, uint32_t y) const
{
...
void
TerminalLabel::set_color(const terminalpp::colour& color)
{
  _color = color;
}
...
TerminalRenderBuffer
TerminalMultilineLabel::render() const
{
...
TerminalRenderBuffer
TerminalProgressBar::render() const
{
  if (_allowed_width < 3)
    return {};
...
TerminalUI::redraw()
{
  ...
  //   max_postprocessed_count =
  //   progress_reporter.get_max_progress<size_t>(progress::POSTPROCESSING);
  ...
  terminal_blit(log_label_rendered, _canvas, 0, 3);

  const auto& log_messages = _state->get_messages();
  uint32_t log_msg_idx = 4;
