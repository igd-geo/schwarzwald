...
parse_memory_size(std::string const& memory_size_string)
{
  ...
  if (suffix_iter == std::end(si_suffix_multipliers)) {
    return tl::make_unexpected(
      (boost::format("Could not parse memory size: Unrecognized SI suffix (%1%). Use one of \"B\", "
...
parseArguments(int argc, char** argv)
{
  ...
  tiler_options.add_options()(
    "source,i",
    bpo::value<std::vector<std::string>>(&source_files),
    ...
    "output-attributes,a",
    bpo::value<std::vector<std::string>>(&tiler_args.output_attributes)->required(),
    ...
    "attribute")(
    "sampling",
    bpo::value<std::string>(&tiler_args.sampling_strategy)->default_value("RANDOM_GRID"),
    "Sampling strategy to use. Possible values are RANDOM_GRID, GRID_CENTER, "
    "MIN_DISTANCE and "
    "MIN_DISTANCE_FAST")(
    "cache-size",
    bpo::value<std::string>(&cache_size_string),
    "Size of a local cache in memory used during conversion for storing points in. You can specify "
    ...
    "Create a detailed journal in the output folder with information about the tiling process. "
  ...
  if (generic_variables["tiler"].as<bool>()) {
    ...
    converter_args.output_attributes = point_attributes_from_strings(output_attributes);
