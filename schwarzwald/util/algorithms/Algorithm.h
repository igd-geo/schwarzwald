 ...
 * to return an iterator pair where the first entry points to the selected
 *element and the second entry is the new begin iterator from which the
 *partitioning should continue. If no match is found by the predicate, both
 *iterators point to the next entry that shall be processed.
 **/
...
stable_partition_with_jumps(const Iter begin, const Iter end, Pred pred)
{
  ...
  const auto tmp_buffer =
    reinterpret_cast<Value_t*>(malloc(sizeof(Value_t) * count));
  const auto tmp_buffer_begin = tmp_buffer;
  const auto tmp_buffer_rbegin =
    std::make_reverse_iterator(tmp_buffer_begin + count);

  // Selected elements are copied in forward sorted order to the beginning of
  // the buffer, unselected elements are copied in reverse sorted order to the
  // end of the buffer. This way, the buffer grows from front and back until
  // eventually both iterators meet
  ...
  while (current != end) {
    ...
    if (selected == next) {
      ...
      unselected_insert_position =
        std::move(current, next, unselected_insert_position);
    } else {
      // Copy everything prior to the selected element to the tmp_buffer, then
      // the selected element to the
      unselected_insert_position =
        std::move(current, selected, unselected_insert_position);
      selected_insert_position =
        std::move(selected, selected + 1, selected_insert_position);
      unselected_insert_position =
        std::move(selected + 1, next, unselected_insert_position);
    }
  ...
  const auto pivot_point =
    std::move(tmp_buffer_begin, selected_insert_position, begin);
 ...
 * and end iterator. If 'num_chunks' does not equally divide the size of the
 * range, the remainder will be in the last chunk. 'num_chunks' has to be >= the
 * size of the range
 */
...
split_range_into_chunks(size_t num_chunks, Iter begin, Iter end)
{
  ...
  for (size_t idx = 0; idx < num_chunks - 1; ++idx) {
    chunks.push_back(std::make_pair(begin + (idx * chunk_size),
                                    begin + (idx + 1) * chunk_size));
  }
...
/**
 * Merge the given range of ranges into a single range. This is the N-ary
 * equivalent to std::merge. It requires that the input ranges are at least
 * forward iterators.
 *
...
void
merge_ranges(util::Range<ForwardIter> in_ranges,
             util::Range<OutputIter> out_range,
             Comp comparator)
{
  using InputRangeType = std::decay_t<decltype(*std::declval<ForwardIter>())>;
  std::vector<InputRangeType> remaining_ranges = { std::begin(in_ranges),
                                                   std::end(in_ranges) };

  /**
   * Looks for the range with the lowest next element in 'remaining_ranges'. If
   * such a range exists, its front element is stored in 'element' and the range
   * is moved to the next element
   */
  const auto get_and_increment_lowest_head =
    [&comparator](std::vector<InputRangeType>& heads, auto& element) {
