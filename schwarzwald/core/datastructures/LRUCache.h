...
template <typename Key, typename Value> struct LRUCache {
  ...
  explicit LRUCache(unit::byte memory_capacity)
      : _memory_capacity(memory_capacity), _free_capacity(memory_capacity) {}

  void put(Key const &key, Value value) {
    assert((concepts::size_in_memory(key) + concepts::size_in_memory(value)) <=
           _memory_capacity);
  ...
  void add_evict_handler(EvictHandler handler) {
    _evict_handlers.push_back(std::move(handler));
  }
...
private:
  ...
  void insert_new(Key const &key, Value value) {
    const auto required_size =
        concepts::size_in_memory(key) + concepts::size_in_memory(value);
  ...
  void update_entry(typename std::deque<Entry>::iterator entry,
                    Value new_value) {
    ...
    const auto size_difference = concepts::size_in_memory(new_value) -
                                 concepts::size_in_memory(tmp_entry.second);
  ...
  typename std::deque<Entry>::iterator find(Key const &key) {
    return std::find_if(
        std::begin(_entries), std::end(_entries),
        [&key](Entry const &entry) { return entry.first == key; });
  }
  ...
  void evict(typename std::deque<Entry>::const_iterator iter) {
    ...
    _free_capacity += concepts::size_in_memory(iter->first) +
                      concepts::size_in_memory(iter->second);
    _entries.erase(iter);
  }
};