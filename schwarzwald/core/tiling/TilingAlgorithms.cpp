                    ...
                    PointsPersistence& persistence)
{
  ...
  /**
   * In a previous version of the code, the MortonIndices were recomputed based
   * on the bounds of the whole octree. Since we have FP inaccuracies, some of
   * our points might be outside of the current node (node_bounds). Naively
   * computing the MortonIndices might be wrong. So we clamped to the bounds of
   * the node to make sure that our points are always inside the node. But even
   * in this case the MortonIndex calculation might result in a wrong
   * MortonIndex, because points on the edge between two nodes can be in one
   * node or the other, depending on the FP to int conversion inside
   * calculate_morton_index
   *
   * What we do instead is use the given MortonIndex of the node as a baseline,
   * and only compute the lower levels based on that index.
   */
  ...
  std::transform(
    ...
    [&](const auto& point_ref) -> IndexedPoint64 {
      ...
      auto morton_index_starting_from_this_node =
        calculate_morton_index<MAX_OCTREE_LEVELS>(point_ref.position(),
                                                  node.bounds);
      ...
      for (auto level = start_level; level < MAX_OCTREE_LEVELS; ++level) {
        idx.set_octant_at_level(
          level,
          morton_index_starting_from_this_node.get_octant_at_level(
            level - start_level));
      }
  ...
  // If the Persistence is lossy, we have to sort, as FP inaccuracies might
  // disturb the order of points
                             ...
                             octree::NodeStructure const& root_node)
{
  ...
  for (uint8_t octant = 0; octant < 8; ++octant) {
    ...
    child_tiling_data.emplace_back(
      octree::NodeData{ std::begin(child_range), std::end(child_range) },
      child_node,
      root_node);
  }
...
static void
dump_tree_to_graphviz(const Tree& tree,
...
{
  // const auto points_to_take = std::min(all_points.size(),
  // _meta_parameters.max_points_per_node); if (points_to_take <
  // all_points.size()) {
  ...
  if (all_points.size() > _meta_parameters.max_points_per_node) {
    util::write_log(
      (boost::format(
         "Taking %1% points at terminal node %2% without sampling") %
  ...
  if (_progress_reporter)
    _progress_reporter->increment_progress(
      progress::INDEXING, all_points.size() - previously_taken_points_count);
}
                                        ...
                                        size_t previously_taken_points_count)
{
  if (all_points.empty()) {
    throw std::runtime_error{
      (boost::format("tile_internal_node: Got zero points to tile @ node %1%") %
       node.name)
        .str()
    };
  }
  /**
   * When we first hit a node with a range of points, we can take all points if
   * their count is less than the maximum number of points per node. Once we had
   * to sample a node for the first time - because of too many points - we
   * ALWAYS have to sample it, because this node now has children with denser
   * sampling. Suddenly picking all points without sampling might result in a
   * parent node with the same (or even higher) density of points as a child
   * node, which is visually unpleasing
   * TODO How costly is this in terms of performance? --> ~20ish%
   * This is a big performance hit, have to see how much it impacts my
   * performance advantage compared to the other tools :/
   */
  const auto sampling_behaviour =
    (previously_taken_points_count > 0)
      ? SamplingBehaviour::AlwaysAdhereToMinSpacing
      : SamplingBehaviour::TakeAllWhenCountBelowMaxPoints;

  const auto node_level_relative_to_root = node.level - (root_node.level + 1);

  const auto partition_point =
    filter_points_for_octree_node(std::begin(all_points),
                                  std::end(all_points),
                                  node.morton_index,
                                  node_level_relative_to_root,
                                  root_node.bounds,
  ...
  if (node_level_relative_to_root >= 16) {
    const auto taken_percentage =
      points_taken / static_cast<double>(all_points.size());
    if (taken_percentage < 0.01) {
      util::write_log(
        concat("Discovered potentially broken node ", node.name, "\n"));
      ...
      if (!fs.is_open()) {
        throw std::runtime_error{
          (boost::format(
             "Could not open file %1% for dumping potentially broken node!") %
           dump_file_path)
            .str()
        };
      }
      ...
      for (auto iter = std::begin(all_points); iter != std::end(all_points);
           ++iter) {
        ...
        fs << position << " [" << *iter->point_reference.point_source_id()
           << "] [" << to_string(morton_idx) << "]\n";
      }
    }
  }
  ...
  return split_range_into_child_nodes(
    partition_point, std::end(all_points), node, root_node);
}
                               ...
                               tf::Subflow& subflow)
{
  ...
  auto cached_points = read_pnts_from_disk(
    node_structure, root_node_structure.bounds, _points_cache, _persistence);
  ...
  const auto node_level_to_sample_from = required_morton_index_depth(
    _sampling_strategy, node_structure.level, root_node_structure);
  const auto requires_deeper_morton_indices =
    node_level_to_sample_from > node_structure.level;
  ...
  const auto max_level = gsl::narrow<int32_t>(
    std::min(MAX_OCTREE_LEVELS - 1, node_structure.max_depth));
  ...
  if (!requires_deeper_morton_indices) {
    if (node_level_to_sample_from >= max_level) {
      ...
      tile_terminal_node(
        all_points_for_this_node, node_structure, cached_points_count);
      return {};
    }

    auto all_points_for_this_node = octree::merge_node_data_sorted(
      std::move(node_data), std::move(cached_points));
    return tile_internal_node(all_points_for_this_node,
                              node_structure,
                              root_node_structure,
                              cached_points_count);
  } else {
    ...
    if (node_level_to_sample_from >= static_cast<int32_t>(MAX_OCTREE_LEVELS)) {

      if (global_config().is_journaling_enabled) {
        journal_string(
          (boost::format("Recalculating Morton indices for deep node %1%%2%") %
           root_node_structure.name % node_structure.name)
            .str());
      }

      // If we are so deep that we exceed the capacity of the MortonIndex,
      // we have to index our points again with the current node as new root
      // node. We also have to carry the information that we have a new root
      // over to the children so that the paths of the nodes are correct.

      // Fun fact: We don't have to adjust the loaded indices because if we
      // ever get to a node this deep again, the indices have been
      // calculated with the new root the last time also, so everything is
      // as it should be
 ...
 */
void
TilingAlgorithmBase::do_tiling_for_node(
  octree::NodeData&& node_data,
  const octree::NodeStructure& node_structure,
  const octree::NodeStructure& root_node_structure,
  tf::Subflow& subflow)
{
  auto child_nodes = tile_node(
    std::move(node_data), node_structure, root_node_structure, subflow);
  ...
  std::for_each(std::begin(child_nodes),
                iter_to_first_sync_node,
  ...
  std::for_each(iter_to_first_sync_node,
                std::end(child_nodes),
                [this, &subflow](NodeTilingData& child_node) {
...
TilingAlgorithmV1::build_execution_graph(
  ...
  tf::Taskflow& tf)
{
  ...
  auto process_task =
    tf.emplace([this, root_node](tf::Subflow& subflow) mutable {
        do_tiling_for_node(
          std::move(_root_node_points), root_node, root_node, subflow);
        _root_node_points = {};
      })
...
std::pair<tf::Task, tf::Task>
TilingAlgorithmV2::build_execution_graph(
  util::Range<PointBuffer::PointIterator> points,
  const AABB& bounds,
  uint32_t num_indexing_threads,
  tf::Taskflow& tf)
    ...
    tf.emplace([this, bounds, num_indexing_threads](tf::Subflow& subflow) {
      ...
      for (auto node : ranges_per_node.traverse_level_order()) {
        ...
        new_tiling_tasks.push_back(subflow.emplace(
          [this, bounds, index = node.index(), _data = std::move(*node)](
...
void
TilingAlgorithmV2::index_and_sort_points(
  ...
  const auto num_points_in_children = [](const Octree_t::MutableNode& node) {
    ...
    return std::accumulate(
      ...
      [](size_t accum, const auto& node) {
        return accum + std::accumulate(std::begin(*node),
                                       std::end(*node),
                                       size_t{ 0 },
  ...
  const auto merge_leaves =
    ...
    for (auto leaf : leaf_parent.children()) {
      ...
      merged_ranges.insert(
        std::end(merged_ranges), std::begin(*leaf), std::end(*leaf));
      ++merged_nodes;
    }
  ...
  const AABB& bounds)
{
  ...
  this_node.name =
    std::string{ "r" } + OctreeNodeIndex64::to_string(node_index);
                                           ...
                                           const AABB& root_bounds)
{
  ...
  const auto selected_points_end =
    sample_points(_sampling_strategy,
                  std::begin(indexed_points),
...
std::pair<tf::Task, tf::Task>
TilingAlgorithmV3::build_execution_graph(
  util::Range<PointBuffer::PointIterator> points,
  const AABB& bounds,
  uint32_t num_indexing_threads,
  tf::Taskflow& tf)
{
  ...
  if (!_level_of_start_nodes.has_value()) {
    return build_execution_graph_for_first_iteration(
      points, bounds, num_indexing_threads, tf);
  } else {
    return build_execution_graph_for_later_iterations(
      points, bounds, num_indexing_threads, tf);
  }
}
  ...
  tf::Taskflow& tf)
{
  ...
  auto index_task = parallel::scatter(
    ...
    [this, chunk_size, bounds](
      ...
      util::Range<PointBuffer::PointIterator> points{ points_begin,
                                                      points_end };
  ...
  auto sort_estimate_get_start_node =
    tf.emplace([this, bounds, num_indexing_threads](tf::Subflow& subflow) {
        util::Range<IndexedPointsIter> indexed_points{
          std::begin(_root_node_points), std::end(_root_node_points)
        };
        indexed_points.sort();

        _level_of_start_nodes = estimate_start_node_level_in_octree(
          indexed_points, num_indexing_threads);
        ...
        auto start_nodes = split_indexed_points_into_subranges(
          indexed_points, *_level_of_start_nodes);
          ...
          subflow
            .emplace(
              [this, bounds, index = node.index(), _data = std::move(*node)](
                ...
                this_node.max_depth = root_node.max_depth;
                this_node.max_spacing =
                  root_node.max_spacing / std::pow(2, index.levels());
                this_node.morton_index = index.to_static_morton_index();
                this_node.name =
                  std::string{ "r" } + OctreeNodeIndex64::to_string(index);

                do_tiling_for_node({ std::begin(_data), std::end(_data) },
                                   this_node,
                                   root_node,
                                   subsubflow);
              })
  ...
  auto transpose_task =
    tf.emplace([this, bounds](tf::Subflow& subflow) {
        auto ranges_per_node =
          merge_selected_start_nodes(_indexed_points_ranges);

        if (global_config().is_journaling_enabled) {
          journal_start_nodes(ranges_per_node.to_graphviz([](const auto& node) {
            std::stringstream ss;
            ss << OctreeNodeIndex64::to_string(node.index()) << " - "
               << std::accumulate(std::begin(*node),
                                  std::end(*node),
                                  size_t{ 0 },
                                  [](auto accum, const auto& range) {
                                    return accum + range.size();
                                  });
            return ss.str();
          }));
        ...
        for (auto node : ranges_per_node.traverse_level_order()) {
          ...
          const auto child_task_name =
            (boost::format("r%1% [%2%]") %
             OctreeNodeIndex64::to_string(node.index()) % num_points)
              .str();
          ...
          subflow
            .emplace(
              [this, bounds, index = node.index(), _data = std::move(*node)](
                tf::Subflow& subsubflow) {
                auto process_data =
                  prepare_range_for_tiling(_data, index, bounds);
...
void
TilingAlgorithmV3::index_and_sort_points(
  util::Range<PointsIter> points,
  util::Range<IndexedPointsIter> indexed_points,
  const AABB& bounds) const
{
  ...
  size_t concurrency) const
{
  ...
  const auto is_large_range =
    [](const util::Range<IndexedPointsIter>& range) -> bool {
    return range.size() >= 100'000;
  };

  const auto score_ranges =
    [concurrency, is_large_range](
    ...
    return static_cast<float>(num_large_ranges) /
           static_cast<float>(concurrency);
  };

  const auto split_at_level =
    [](uint32_t level,
    ...
    for (const auto& range : ranges) {
      const auto split_ranges = partition_points_into_child_octants(
        std::begin(range), std::end(range), level);
  ...
  size_t level_of_subranges_in_octree) const
{
  ...
  while (!next_nodes.empty()) {
    ...
    const auto child_ranges = partition_points_into_child_octants(
      std::begin(*node), std::end(*node), node_index.levels());
    for (uint8_t octant = 0; octant < 8; ++octant) {
      const auto& child_range = child_ranges[octant];
      if (child_range.size() == 0) {
        continue;
...
void
TilingAlgorithmV3::reconstruct_single_node(const OctreeNodeIndex64& node,
                                           const AABB& root_bounds)
{
  PointBuffer data;
  for (uint8_t octant = 0; octant < 8; ++octant) {
...
TilingAlgorithmV3::reconstruct_left_out_nodes(const AABB& root_bounds)
{
  ...
  const auto node_exists = [this](const OctreeNodeIndex64& node_index) {
    const auto node_name =
      concat("r", OctreeNodeIndex64::to_string(node_index));
  ...
  std::vector<OctreeNodeIndex64> sorted_nodes_to_reconstruct{
    std::begin(nodes_to_reconstruct), std::end(nodes_to_reconstruct)
  };
  std::sort(
    std::begin(sorted_nodes_to_reconstruct),
