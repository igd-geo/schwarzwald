  ...
  switch (tokens.size()) {
    case 1:
      return util::try_parse_number<uint32_t>(tokens[0])
        .map([](uint32_t num_threads) {
          return ThreadConfig{ AdaptiveThreadCount{ num_threads } };
        })
        .map_error([](const auto& err) {
          return std::visit(overloaded{
                              ...
                              [](const std::out_of_range& ex) {
                                return "Thread count is out of range";
                              },
                            },
                            err);
        });
        ...
        .and_then([&tokens](uint32_t read_threads) {
          return util::try_parse_number<uint32_t>(tokens[1]).map(
            [read_threads](uint32_t index_threads) {
              return ThreadConfig{ FixedThreadCount{ read_threads,
                                                     index_threads } };
            });
        })
        .map_error([](const auto& err) {
...
parseArguments(int argc, char** argv)
{
  ...
  tiler_options.add_options()(
    ...
    "MIN_DISTANCE, JITTERED. The quality of the resulting point cloud can be "
    "adjusted "
    "with this parameter, with RANDOM_GRID corresponding to the lowest "
    "quality and MIN_DISTANCE to the highest quality. JITTERED gives similar "
    "quality to MIN_DISTANCE, but with better performance.")(
    "calculate-rgb-from",
    ...
    "threads",
    bpo::value<std::string>(),
    "The number of threads to use. Specify either a single number to use this "
    "many threads with "
    "adaptive scheduling (the threads will be distributed between reading and "
    "indexing "
    "dynamically), or specify two numbers for the reading threads and indexing "
    "threads, in this "
    "order. E.g.: \"--threads 6\" will use 6 threads, dynamically assigned to "
    "reading and "
    "indexing. \"--threads 2 6\" will use a total of 8 threads, 2 for reading, "
    "6 for indexing. If "
    "unspecified, as many threads as there are logical cores on the current "
    "machine will be used "
    "and dynamically assigned to reading and indexing. Please not that reading "
    "is parallelized on "
    "a file-level, so there can never be more read threads than there are "
    "files.");
  ...
  if (generic_variables["tiler"].as<bool>()) {
    ...
    if (tiler_variables.count("threads")) {
      ...
      parse_threads_count(tiler_variables["threads"].as<std::string>())
        .map([&tiler_args](const auto& thread_config) {
          tiler_args.thread_config = thread_config;
        })
      ...
      // Debatable what the best ratio of read to index threads is. Will be
      // adjusted by the TilerProcess based on the actual number of files
      // anyways. We will use 1 thread for reading and the rest for indexing
