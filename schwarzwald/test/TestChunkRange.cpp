...
TEST_CASE("Chunk range generates even chunks when chunk count is a divider of "
          "range size",
  ...
  const auto actual_chunks =
      split_range_into_chunks(NumChunks, range.begin(), range.end());

  REQUIRE(actual_chunks == expected_chunks);
}

TEST_CASE("Chunk range covers full range if chunk count is not a divier of "
          "range size",
          "[split_range_into_chunks]") {
  constexpr size_t NumChunks = 4;
